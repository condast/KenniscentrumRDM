 ...
 *******************************************************************************/
package org.rdm.authentication.utils;
...
public class StringStyler 
{
	...
	*/
	 ...
	 * @return
	 */
	public static String styleFromXML( String strng ){
		String first = strng.substring(0, 1).toUpperCase();
		StringBuffer buffer = new StringBuffer();
		buffer.append( first );
		int i=1;
		while( i<strng.length()){
			char chr = strng.charAt(i);
			if(( chr == '-' ) && ( i < strng.length()-1)){
				buffer.append( " " );
				i++;
				buffer.append( strng.substring(i, i+1).toUpperCase());
			}else
				buffer.append(chr);
			i++;
		}
